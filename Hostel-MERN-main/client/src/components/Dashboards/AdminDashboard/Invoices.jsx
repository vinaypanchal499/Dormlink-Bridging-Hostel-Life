...
function Invoices() {
  ...
  const genInvoices = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success("Invoices generated successfully!", { position: "top-right", autoClose: 3000, theme: "dark" });
        getInvoices();
      } else {
        toast.error(data.errors, { position: "top-right", autoClose: 3000, theme: "dark" });
      }
    } catch (err) {
      toast.error(err.errors, { position: "top-right", autoClose: 3000, theme: "dark" });
    }
    setProgress(100);
  };
  ...
  const approveInvoice = async (id) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success("Invoice approved successfully!", { position: "top-right", autoClose: 3000, theme: "dark" });
        getInvoices();
      } else {
        toast.error("Something went wrong!", { position: "top-right", autoClose: 3000, theme: "dark" });
      }
    } catch (err) {
      toast.error("Something went wrong!", { position: "top-right", autoClose: 3000, theme: "dark" });
    }
  ...
  return (
    <div className="w-full min-h-screen flex flex-col gap-5 items-center justify-start py-10 bg-gray-900">
      ...
      {/* Updated box with columns */}
      <div className="bg-neutral-950 px-10 py-8 rounded-xl shadow-xl sm:w-[90%] sm:min-w-[700px] w-full mt-5 max-h-[700px] overflow-auto">
        <span className="text-white font-bold text-xl mb-4 block">Pending Invoices</span>

        {/* Table headers */}
        <div className="grid grid-cols-5 gap-4 text-gray-400 font-semibold mb-2 px-2">
          <span>Name</span>
          <span>Room</span>
          <span>Amount</span>
          <span>Status</span>
        ...
        <ul role="list" className="divide-y divide-gray-700 text-white">
          {pendingInvoices.length === 0
            ? <p className="text-center text-white py-4">No Pending Invoices</p>
            : pendingInvoices.map((invoice) => (
                <li
                  className="py-3 px-2 rounded hover:bg-neutral-700 hover:scale-105 transition-all"
                  key={invoice.id}
                >
                  <div className="grid grid-cols-5 gap-4 items-center">
                    <span className="truncate">{invoice.student.name}</span>
                    <span className="truncate">{invoice.student.room_no}</span>
                    <span className="truncate">Rs. {invoice.amount}</span>
                    <span className="truncate">{invoice.status}</span>
                    <button
                    ...
                    >
                      ...
                      <span className="text-sm hidden absolute px-2 -right-10 top-6 bg-black text-center group-hover:block rounded">
                        Approve Payment
                      </span>
                    </button>
                  </div>
